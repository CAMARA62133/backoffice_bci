...
export class HistoriqueTransactionComponent implements OnInit {
      ...
      .subscribe({
        next: (response: any) => {
          if (response.status === 200) {
            this.toastr.success('Transaction annulée avec succès', 'Succès', {
              positionClass: 'toast-custom-center',
            });
            this.loadHistorique();
          } else {
  ...
  checkStatus(item: any): void {
    if (!item.iRequestID)  {
      this.toastr.warning('Identifiant manquant', 'Erreur', {
        positionClass: 'toast-custom-center',
      });
      return;
    }
        ...
        next: (res: any) => {
          if (res && res.status === 200) {
            ...
            if (nouveauStatut !== item.Status) {
              ...
              this.toastr.info('Le statut n’a pas changé', 'Info');
            }
          } else {
            this.toastr.error(
              this.utils.decodeMessage(res?.message) || 'Erreur API',
            );
          }
        },
        error: () => this.toastr.error('Erreur réseau', 'Erreur'),
      });
  }
}
