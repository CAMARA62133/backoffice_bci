...
//       this.notification.error('Aucune organisation sélectionnée.');
...
export class OrganisationsComponent implements OnInit {
  ...
  onlyDigits(event: KeyboardEvent) {
    ...
    if (
      !/^[0-9]$/.test(event.key) ||
      (input.value.length === 0 && event.key !== this.phoneFirstNumber) ||
      input.value.length >= this.phoneMaxLength
    ) {
      event.preventDefault();
    }
  }
  ...
  onPaste(event: ClipboardEvent) {
    ...
    const input = event.target as HTMLInputElement;
    const regex = new RegExp(`^\\d{1,${this.phoneMaxLength}}$`);
    if (!regex.test(pastedData)) event.preventDefault();
    const finalValue = input.value + pastedData;
    if (finalValue.length > 0 && finalValue.charAt(0) !== this.phoneFirstNumber)
      event.preventDefault();
    if (input.value.length + pastedData.length > this.phoneMaxLength)
      event.preventDefault();
  }
  ...
  openCreateModal() {
    ...
    this.initForm();
    this.orgForm.reset();
    this.modalsService.openModal(this.createOrgModalId);
  }
  ...
  openModal(modalId: string) {
    ...
    if (!isModalOpen) {
      this.modalsService.openModal(modalId);
      // On ne reset le formulaire que si ce n'est pas la modal de blocage
      if (modalId !== 'bloquerDebloquerModal') {
        this.orgForm.reset();
      }
    }
    // SUPPRIMEZ CES LIGNES ICI :
    // this.modalsService.closeModal(modalId);
    // this.modalsService.closeAllModals();
  }
  closeModal(modalId: string) {
  ...
  onSubmit() {
    ...
    this.orgService.createOrganisation(dataToSend).subscribe({
      next: (res) => {
        if (res?.status === 200) this.notification.success(res.message);
  ...
  onEdit(org: any) {
    ...
    this.idUsers = org.idUsers;

    this.orgForm.patchValue({
      vcOrgName: org.organisationName,
      vcOrgContact: org.vcContact,
      vcOrgPhoneNumber: org.vcPhoneNumber,
      vcOrgEmail: org.vcEmail,
      vcOrgCity: org.vcCity,
      vcOrgCountry: org.vcCountry,
      vcOrgAddress: org.vcAddress,
      vcOrgLogoPath: '',
      vcBusinessEmailDomain: org.vcBusinessEmailDomain,
      vcFirstname: org.vcFirstnameUsers,
      vcLastname: org.vcLastnameUsers,
      vcDescription: org.vcDescription,
      iRoleID: org.iRoleID,
      modeOtp: org.modeOtp || 'Email',
    });
  ...
  bloquerEtDebloquer(): void {
    ...
    this.orgService.toggleActiveOrDesactiveOrg(params).subscribe({
      next: (res) => {
        // 3. Arrêter le chargement immédiatement
        this.isloadingBloquerDebloquer = false;

        if (res?.status === 200) {
          this.notification.success(res?.message);

          // 4. MISE À JOUR LOCALE (Pour changer le cadenas instantanément)
          const org = this.organisations.find(
            (o) => o.id === this.selectedOrgId,
          );
          if (org) {
            org.btEnabled = this.btEnabled; // Met à jour l'icône dans le *ngFor
          }
          console.log(res.data);
          // 5. Fermeture et rafraîchissement DataTables
          this.modalsService.closeModal('bloquerDebloquerModal');
          this.loadOrganisations(); // Recharge les données via rerender()
        } else {
          this.notification.error(res?.message || 'Une erreur est survenue');
        }
      },
      error: (err) => {
        this.isloadingBloquerDebloquer = false;
  ...
  // ===========================
  // LOGIQUE TABLE (FILTRAGE, TRI, PAGINATION)
  // ===========================
  get filteredData() {
